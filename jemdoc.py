...
#      eqtext = eqtext.replace('\\', '')
            ...
            if wl:
                ...
                b = b[:m.start()] + 'BACKSLASH65358OPENBRACKET65358' + \
                    eqtext + 'BACKSLASH65358CLOSEBRACKET65358' + b[m.end():]
                ...
                b = b[:m.start()] + 'BACKSLASH65358(' + eqtext + \
                    'BACKSLASH65358)' + b[m.end():]
...
def replacelinks(b):
    ...
    while m:
        ...
        b = b[:m.start()] + r'<a href=\"%s\"%s>%s<\/a>' % (link,
                                                           option, linkname) + b[m.end():]
        ...
        if l.startswith('# jemdoc:'):
            ...
            for b in a:
                ...
                if b.startswith('menu'):
                    ...
                    if len(g) != 1:
                        raise SyntaxError(
                            'addtitle error on line %d' % f.linenum)
                    ...
                    if len(g) != 1:
                        raise SyntaxError(
                            'eqsize error on line %d' % f.linenum)
